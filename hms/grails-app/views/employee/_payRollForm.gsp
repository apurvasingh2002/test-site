    ...
    <div class="modal-content">
            ...
            <g:form id="gForm"
                ...
                <table class="form-table">
                    <tr>
                        ...
                        <td><label>Employee: <g:select name="employee" from="${hrm.Employee.list()}"
                                                       optionValue="firstName" optionKey="id" required=""
                                                       value="${objectInstance?.employee?.id}"
                                                       noSelection="${[null: 'Select One...']}"></g:select></label></td>
                        <td></td>

                    </tr>
                    <tr>
                        <td><label>Promotion Date:
                            <input name="promotionDate" placeholder="Promotion Date" id="promotionDate"
                                   data-date-format="dd/mm/yyyy" data-provide="datepicker"
                                   value="${formatDate(format: 'dd/MM/yyyy', date: objectInstance?.promotionDate)}">
                        </label>
                        </td>
                        <td><label>Salary: <g:textField name="salary" required=""
                                                        value="${objectInstance?.salary}"/></label></td>
                    </tr>
        ...
        <div class="modal-footer">
            <button type="button" class="save btn btn-primary"
                    onclick="submitForm('payRoll', 'Payroll')">${objectInstance.id ? 'Update' : 'Save'}</button>
